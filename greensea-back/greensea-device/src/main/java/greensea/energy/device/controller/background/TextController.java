 ...
 * @Description:
 * @Author: gmslymhn
 * @CreateTime: 2024-08-02 18:08
 * @Version: 1.0
 **/
@RestController
@Tag(name = "修改前端文字")
@RequestMapping("background/text")
@Slf4j
public class TextController {
    ...
    public R getTextList(@RequestBody @Validated PageParam pageParam) {
        return iTextService.getTextList(pageParam);
    }
    @PostMapping("/updateText")
    @PreAuthorize("@ss.hasPermission('admin')")
    @SysLogAnnotation(operModul = "前台管理>>前端文字管理", operType = "修改", operDesc = "修改前端文字")
    @Operation(summary = "修改前端文字")
    public R updateText(@RequestBody @Validated TextEntity textEntity) {
        return iTextService.updateText(textEntity);
    }
}
