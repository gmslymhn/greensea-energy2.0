...
import greensea.energy.upload.utils.http.AddressUtil;
...
public class TokenService {
    ...
    public void updateDeviceMsg(UploadDto uploadDto){
        redisUtils.setCacheObject(DEVICE_MESSAGE+uploadDto.getDeviceNumber(),uploadDto);
    }
    public void updateDeviceAddress(UploadDto2 uploadDto2){
        redisUtils.setCacheObject(DEVICE_ADDRESS+uploadDto2.getDeviceNumber(),uploadDto2);
    }
    ...
    public String refreshToken(Device device, DeviceToken deviceToken) {
        ...
        if (ObjectUtils.isNull(device.getAddress())){
            device.setAddress(AddressUtil.getAddressByIP(deviceToken.getDeviceIp()));
        }
